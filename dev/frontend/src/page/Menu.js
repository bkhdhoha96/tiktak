  ...
  return (
    <div className="p-2 md:p-4">
      ...
      {productDisplay && productDisplay.image && (
        ...
        {productDisplay && productDisplay.name ? productDisplay.name : 'No Name Available'}
      </h3>
      <p className="text-slate-500 font-medium text-2xl">
      {productDisplay && productDisplay.category ? productDisplay.category : 'No Category Available'}
    </p>
    <p className="font-bold md:text-2xl">
  <span className="text-red-500">₹</span>
  <span>{productDisplay?.price ?? 'No Price Available'}</span>
</p>
